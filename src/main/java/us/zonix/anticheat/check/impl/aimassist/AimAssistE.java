...
public class AimAssistE extends RotationCheck
{
    ...
    public AimAssistE(final LordMeme plugin, final PlayerData playerData) {
    ...
    public void handleCheck(final Player player, final RotationUpdate update) {
        ...
        final float diffYawPitch = Math.abs(diffYaw - diffPitch);
        ...
        if (diffYaw > 0.05f && diffPitch > 0.05 && (diffPitchRate > 1.0 || diffYawRate > 1.0) && (diffPitchRatePitch > 1.0f || diffYawRateYaw > 1.0f) && diffYawPitch < 0.009f && diffYawPitch > 0.001f) {
            this.alert(PlayerAlertEvent.AlertType.EXPERIMENTAL, player, String.format("DYP %.3f. DP %.3f. DY %.3f. DPR %.3f. DYR %.3f. DPRP %.3f. DYRY %.3f.", diffYawPitch, diffYaw, diffPitch, diffPitchRate, diffYawRate, diffPitchRatePitch, diffYawRateYaw), false);
        }
